...
struct Process* pheap_remove_top()
{
   ...
   printf("Process removed from ready queue. Size is %d\n",pheap_size);
   p_heapify_down(0);
   return top;
}
...
void pheap_insert(struct Process* temp)
{
    ...
    pheap_size++;
    printf("New process inserted in ready queue. Size is %d\n",pheap_size);
...
struct Process* newProcess(int pid, char state, int arrival_time, int cpu_burst)
{
    ...
    temp->last_inserted=0;              //Time at which process was last inserted into ready queue
	return temp;
}
    ...
    while(eheap_size>0)
    {
        ...
        if(current_event->eventType==0)
        {
            ...
            if(pheap_size>0)
            {
                ...
                current_process=pheap_remove_top();
        ...
        else if(current_event->eventType==1)
        {
            ...
            if(CPU==current_event->pid)                                                                            //Time out occurs when the process is running
                {
                    printf("Time Out\n");                                                                           //Give the alert
                    current_process->state='0';                                                                     //Ready State
                    current_process->last_inserted=current_time;
                    pheap_insert(current_process);                                                                  //Insert the running process into the ready queue
                    current_process=pheap_remove_top();                                                             //Remove a process using RR
                    current_process->wait_time+=(current_time-current_process->last_inserted);                      //Calculate how long was it in the queue
        ...
        else
        {
            ...
            {
                struct Event* timer_event=newEvent(current_process->pid,1,current_time+quantum);    //Make a timer condition and insert in heap
                eheap_insert(timer_event);
                current_process=newProcess(current_event->pid,'1',arrival_time,burst_time);     //we make a processfor completion but don't add it to queue
                CPU=current_event->pid;                                                         //Since CPU was idle, the process is sent for execution
            }

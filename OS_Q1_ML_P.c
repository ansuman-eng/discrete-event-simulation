...
int main()
{
	...
	while(eheap_size>0)
	{
		...
		if(current_event->eventType==0)
		{
            ...
            if(CPU==current_event->pid && (((current_time-current_process->arrival_time)-current_process->wait_time)==current_process->cpu_burst))                 //CHECK IF THE PROCESS CORRESPONDING TO THE EVENT IS THERE IN THE CPU OR NOT
            {
